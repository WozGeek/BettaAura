...
def setup(
    ...
    # Ensure agent_permissions key exists in config (non-destructive)
    try:
        ensure_default_permissions()
    except Exception:
        pass
...
@app.command()
def permissions(
    action: str = typer.Argument("list", help="Action: list | set | reset"),
    agent: Optional[str] = typer.Argument(None, help="Agent name (for 'set' action): claude, cursor, chatgpt, gemini…"),
    packs_arg: Optional[str] = typer.Argument(None, help="Comma-separated pack names, or 'all' (for 'set' action)"),
):
    """Manage per-agent pack permissions.

    \b
    Examples:
      aura permissions list
      aura permissions set claude developer,writer
      aura permissions set cursor developer
      aura permissions set chatgpt all
    ...
    if action == "list":
        ...
        if not perms:
            rprint(Panel.fit(
                "[dim]No per-agent permissions configured.[/dim]\n\n"
                "  All agents see all packs (default behaviour).\n\n"
                "  [dim]aura permissions set claude developer,writer[/dim]",
                title="aura permissions",
                border_style="cyan",
            ))
            return

        rprint(Panel.fit("[bold]✦ aura permissions[/bold]", border_style="cyan"))
        tbl = Table(show_header=True, header_style="dim", box=None, padding=(0, 2))
        tbl.add_column("Agent", style="cyan")
        tbl.add_column("Allowed packs")
        for ag, rule in perms.items():
            if rule == "all":
                tbl.add_row(ag, "[dim]all packs[/dim]")
            elif isinstance(rule, list):
                tbl.add_row(ag, ", ".join(rule))
            else:
                tbl.add_row(ag, str(rule))
        console.print(tbl)
        rprint("\n  [dim]Run 'aura permissions reset' to restore default (all packs visible to all agents).[/dim]")

    elif action == "set":
        if not agent:
            rprint("[red]✗ Provide an agent name: aura permissions set <agent> <packs>[/red]")
            raise typer.Exit(1)
        if not packs_arg:
            rprint("[red]✗ Provide pack names or 'all': aura permissions set claude developer,writer[/red]")
            raise typer.Exit(1)

        if packs_arg.strip().lower() == "all":
            set_agent_all(agent)
            rprint(f"[green]✦ {agent}[/green] → all packs")
        else:
            pack_list = [p.strip() for p in packs_arg.split(",") if p.strip()]
            set_agent_permissions(agent, pack_list)
            rprint(f"[green]✦ {agent}[/green] → {', '.join(pack_list)}")

    elif action == "reset":
        reset_permissions()
        rprint("[green]✦ Permissions reset.[/green] All agents now see all packs.")

    else:
        rprint(f"[red]✗ Unknown action '{action}'.[/red] Use: list | set | reset")
        raise typer.Exit(1)
