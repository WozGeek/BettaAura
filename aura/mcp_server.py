...
def _filter_packs(packs, agent: str = "unknown"):
    ...
    # Per-agent permissions layer
    return filter_packs_for_agent(packs, agent)
...
# Tool execution
# ---------------------------------------------------------------------------
def execute_tool(name: str, arguments: dict, agent: str = "unknown") -> list[dict]:
    # Allow agent_id override from tool arguments
    if "agent_id" in arguments:
        agent = str(arguments["agent_id"])
...
def handle_jsonrpc(data: dict, agent: str = "unknown") -> dict | None:
    method = data.get("method", "")
    rid = data.get("id")
    params = data.get("params", {})

    # Agent can also be declared inside the params at the JSON-RPC level
    if isinstance(params, dict) and "agent_id" in params:
    ...
    if method == "initialize":
        ...
        content = execute_tool(params.get("name", ""), args, agent=agent)
...
@app.get("/health")
async def health():
    packs = _filter_packs(list_packs(), agent="unknown")
